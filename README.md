...
The figure below shows the comparison of these metrics 
<br>
![image](https://github.com/user-attachments/assets/70b06b71-fd4a-4a5c-a750-1b05eb5ec7e6)

If we look closely in the overall accuracy, the DES classifiers produce higher accuracy compared to static ensemble classifiers, however, cross weighted entropy loss tells a different stories. Both KNORA-E and METADES produce a higher loss, indicating that the model are less confident in their classification probabilites. Similar pattern can be seen in Geometric Mean score, in which DES classifier falls behind Catboost, Rotfor, and XGB. These values can be caused by the nature of the pool classifiers, in which they are trained on smaller subset of the training data. Additionally, the static ensemble classifiers were optimized by conducting recursive feature elimination, boosted their predictive performance. This is one of the limitation of DES classifiers, in which the pool classifiers need to have the same feature set. 
For further testing their predictive capabilities, a separate accuracy assessment is conducted for the LULC maps, using a new validation data, not used for model building procedure.

## Thematic Accuracy Assessment
Thematic acccuracy assessment is conducted for providing a more comprehensive evaluation on the LULC maps generated by these classifiers. These additional steps is recommended by [Olofsson et al 2014](https://doi.org/10.1016/j.rse.2014.02.015) for best practice in accuracy assessment. Furthermore, [Lyons et al 2018](https://doi.org/10.1016/j.rse.2018.02.026) argue against the use of single split procedure (train and test split like before), for accuracy assessment since the validation data required a certain statistical requirement for rigorous accuracy assessment. The procedure for this assessment is similar to model evaluation, with the notable difference in the prediction and testing data. Below are the example script for conducting the assessment:

```python 
def accuracy_assessment(reference, lulc_paths, nodata_value=255):
    # Load the reference raster
    with rasterio.open(reference) as ref_src:
        reference_raster = ref_src.read(1)
    
    # Mask NoData values
    valid_mask = reference_raster != nodata_value
    reference_valid = reference_raster[valid_mask].flatten()  # Flatten for metrics
    # Compute class weights only once
    unique_classes = np.unique(reference_valid)
    weights = compute_class_weight(class_weight='balanced', classes=unique_classes, y=reference_valid)
    # Store in a dictionary for easy lookup
    class_weight_dict = dict(zip(unique_classes, weights))
    print(f"Computed Class Weights: {class_weight_dict}")
    # Convert reference labels to their corresponding weights
    sample_weights = np.array([class_weight_dict.get(cls, 1) for cls in reference_valid])
    # Initialize results dictionary
    result = {}

    # Loop through all land cover maps
    for lulc, pred_path in lulc_paths.items():
        with rasterio.open(pred_path) as pred_src:
            predicted_raster = pred_src.read(1)  # Read predicted raster
        
        # Apply mask and flatten
        predicted_valid = predicted_raster[valid_mask].flatten()

        # Compute accuracy metrics
        oa = accuracy_score(reference_valid, predicted_valid)
        f1 = f1_score(reference_valid, predicted_valid, average = 'weighted', sample_weight=sample_weights)
        gmean = geometric_mean_score(reference_valid, predicted_valid, average = 'weighted', sample_weight=sample_weights)
    ...
    result_df.rename(columns={'index': 'Classifiers'}, inplace=True)
    print(result_df)
    # Save report to CSV
    output_path = "C:\Master of Remote Sensing\Python Code\EL_Research\Analysis_EL\Thematic_accuracy_New_EL.csv"    
    result_df.to_csv(output_path, index=True)  
    return result_df
```
We can applied this function by defining the validation data and LULC data, in which both has the same format (raster)

```python

#define the validation data
test_sites = 'C:\Master of Remote Sensing\Python Code\Basic_ML_research\Validation_sites\Valid_rast_1.tif'
#define the LULC maps
lulc_data = {
'XGBoost': 'C:/Master of Remote Sensing/Python Code/EL_Research/NewOutput/XGB_Final.tif',
'ERT': 'C:/Master of Remote Sensing/Python Code/EL_Research/NewOutput/ERT_Final.tif',
'Rotfor': 'C:/Master of Remote Sensing/Python Code/EL_Research/NewOutput/Rotfor_final.tif',
'Catboost': 'C:/Master of Remote Sensing/Python Code/EL_Research/NewOutput/CB_final.tif',
'KNORA-E':'C:/Master of Remote Sensing/Python Code/EL_Research/NewOutput/New_KNORA-E.tif',
'METADES':'C:/Master of Remote Sensing/Python Code/EL_Research/NewOutput/New_METADES.tif',
}
#define the class name
class_names = [
    'Settlement Build Up', 'Non Settlement Build Up', 'Volcanic Rock and Sand', 'Lowland Forest', 'Upland Forest', 
    'Mangrove Forest', 'Marine Sand', 'Grassland', 'Production Forest', 'Aquaculture', 'Dryland Agriculture', 
    'Natural Open field', 'Man Made Open field', 'Rubber and other hardwood', 'Ocean waters', 'Oil Palm Plantation', 
    'Tea Plantation', 'Bushes and Shrubs', 'Rivers', 'Wetland Agriculture', 'Natural Freshwaterbody'
]
accuracy_result = accuracy_assessment(test_sites, lulc_data)
```
The result can be plotted using matplot lib

![image](https://github.com/user-attachments/assets/e0d094af-abd2-4218-8c24-4b999a7d13c2)

## Closing Remarks
The script and workflow i provide here are not perfect, and in the future i planned to improve the workflow for model learning, as well as model evaluation. I hope this writing provide you with the example workflow of machine learning classification, primarily using some more advanced ensemble learning model. The script i used here were created with the assisstance of various LLM model, since currently i only have applicable knowledge of python programming. I look forward for critics and input from you all. You could reach out to me via my linkedin [profile](https://www.linkedin.com/in/agilakbarfahrezi11/), or email: agilfahrezy60@gmail.com
